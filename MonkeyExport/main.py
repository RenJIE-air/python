...
def getMonkeyLog(SN_list):
    ...
    if len(SN_list) == 1:
        ...
        # showMessage(cmd_ADB + cmd_monkeylog + outPath + SN_list[0] + " 正在导出，请稍后...")
        ...
        for SN in SN_list:
            ...
            if state == 0:
                # showMessage("monkeylog导出成功,文件已保存至[" + outPath + SN + "\\monkeylog.txt]")
                global_ms.text_print.emit("monkeylog导出成功,文件已保存至[" + outPath + SN + "\\monkeylog.txt]")
            else:
                SN_fail_list.append(SN)
                # showMessage("monkeylog导出失败，已停止导出设备[" + SN + "]的相关文件，请确认设备是否正常进行了monkey测试。")
...
def getMonkeyLogError(SN_list):
    state = 1
    if len(SN_list) == 1:
        # showMessage(cmd_ADB + cmd_monkeylog_error + outPath + SN_list[0] + " 正在导出，请稍后...")
        ...
        if state == 0:
            # showMessage("monkeylog_error导出成功,文件已保存至[" + outPath + SN_list[0] + "\\monkeylog_error.txt")
            global_ms.text_print.emit("monkeylog_error导出成功,文件已保存至[" + outPath + SN_list[0] + "\\monkeylog_error.txt]")
        else:
            # showMessage("monkeylog_error导出失败，程序结束，请确认设备是否正常进行了monkey测试。")
            global_ms.text_print.emit("monkeylog_error导出失败，程序结束，请确认设备是否正常进行了monkey测试。")
    else:
        for SN in SN_list:
            # showMessage(cmd_ADB + "-s " + SN + cmd_monkeylog_error + outPath + SN + " 正在导出，请稍后...")
            ...
            if state == 0:
                # showMessage("monkeylog_error导出成功,文件已保存至[" + outPath + SN + "\\monkeylog_error.txt]")
                global_ms.text_print.emit("monkeylog_error导出成功,文件已保存至[" + outPath + SN + "\\monkeylog_error.txt]")
            else:
                SN_fail_list.append(SN)
                # showMessage("monkeylog_error导出失败，已停止导出设备[" + SN + "]的相关文件，请确认设备是否正常进行了monkey测试。")
...
def monkeylog_analyse(monkeylog_error_path, SN_list):
    ...
    for SN in SN_list:
        ...
        result_anr.clear()
        monkey_calendar = ""
        # showMessage("开始分析设备：[" + SN + "]monkey结果")
        ...
        try:
            ...
            # showMessage("设备：[" + SN + "]" + monkey_calendar)
            ...
            for key, value in result_anr.items():
                result_file.write(key + " " + str(value) + "\n")
            result_file.close()
            # showMessage("设备：[" + SN + "]ANR结果统计完成")
            global_ms.text_print.emit("设备：[" + SN + "]ANR结果统计完成")
            # showMessage("设备：[" + SN + "]结果分析完成，结果已保存至[" + monkeylog_error_path + "result.txt]")
            global_ms.text_print.emit("设备：[" + SN + "]结果分析完成，结果已保存至[" + monkeylog_error_path + "result.txt]")
        except FileNotFoundError:
            # showMessage("没有找到monkeylog文件！")
...
# def getLogcat(outPath, SN_list):
#     if not SN_list:
#         # showMessage("logcat-未连接设备，请连接后重试")
#         global_ms.text_print.emit("logcat-未连接设备，请连接后重试")
#     for SN in SN_list:
#         if len(SN_list) > 1:
#             state = os.system(cmd_ADB + " -s " + SN + cmd_logcat + " " + outPath + SN)
#             if state == 0:
#                 # showMessage("设备：[" + SN + "]logcat导出成功")
#                 global_ms.text_print.emit("设备：[" + SN + "]logcat导出成功")
#             else:
#                 # showMessage("设备：[" + SN + "]logcat导出失败")
#                 global_ms.text_print.emit("设备：[" + SN + "]logcat导出失败")
#         elif len(SN_list) == 1:
#             state = os.system(cmd_ADB + cmd_logcat + " " + outPath + SN)
#             if state == 0:
#                 # showMessage("设备：[" + SN + "]logcat导出成功")
#                 global_ms.text_print.emit("设备：[" + SN + "]logcat导出成功")
#             else:
...
def exportAll():
    ...
    # if exportLogCat and successMonkey == 0:
    #     Thread(target=getLogcat, args=(outPath, SN_list)).start()
...
def getInfo():
    ...
    # exportLogCat = ui.logcatCheck.isChecked()
    if outPath[-1] != "\\":
        outPath = outPath + "\\"
...
def get_devices():
    ...
    if len(result) > 1:
        ...
        ui.exportButton.setEnabled(False)
        ui.DevicesText.setPlaceholderText("未连接设备!请连接设备或检查ADB调试是否开启后点击刷新。")
...
if __name__ == '__main__':
    ...
    window.setWindowTitle("MonkeyExport-renjie_2.0")
    window.show()
    sys.exit(app.exec_())
